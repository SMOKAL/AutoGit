...
Rule7ReaestateProof as 
  (Select t.id,t.pandate,p.Registrationid from tea t join
   cte4 p)IamChangingThisCode1234
...
Rule8SibblingData as 
  (Select s.name,s.sibling from Sister s where s.id in ('234','567','678') )This is the newcode123
...
Rule4HomeAddress as
 ...
 from home h join
      address a
 on h.plotid=a.id)CICDTestingWithDifferentOp
